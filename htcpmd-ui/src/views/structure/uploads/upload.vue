<template>
    <d2-container class="structure-import-container">
        <!-- <template slot="header">区域划分</template> -->
        <el-row type="flex" :gutter="20" class="upload-wrap">
            <el-col :xl="8" :lg="12">
                <!-- upload box -->
                <el-upload class="upload-structure-box" drag multiple ref="structureUpload" :action="uploadUrl"
                    :headers="uploadHeaders" :on-preview="handleFilePreview" :on-change="handleFileChange"
                    :on-remove="handleFileRemove" :file-list="fileList" :auto-upload="false" :on-error="uploadError"
                    :on-success="uploadSuccess" :on-exceed="replaceCurrentFile">
                ...
                <el-row class="d2-pl-30">
                    <el-button type="warning" plain @click="clearFileList">Clear Current Files List</el-button>
                    <el-button type="primary" plain @click="uploadSubmit">Submit To Server</el-button>
                </el-row>
            </el-col>
...
const apiPrefix = `/api/${setting.api.htcpmd_center}/`
export default {
    ...
    data() {
        return {
            ...
            fileList: [],
            uploadUrl: `${apiPrefix}structure/upload/`,
            uploadHeaders: {
                Authorization: `${process.env.VUE_APP_TOKEN_PREFIX} ${util.cookies.get("token")}`
            }
        };
    },
    mounted() {
    ...
    methods: {
        ...
        uploadSuccess(resp) {
            this.$message.success(resp && resp.msg);
        },
        uploadError(err) {
